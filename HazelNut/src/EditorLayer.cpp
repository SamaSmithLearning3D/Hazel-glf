...
#define PROFILE_SCOPE(name)  Timer timer##__LINE__(name,[&](ProfileResult profileRusult) {m_ProfileResults.push_back(profileRusult); })
	...
	void EditorLayer::OnAttach()
	{
		...
		auto bb = m_CameraEntity.AddComponent<CameraComponent>();
	...
	{
		if (Framebufferspecification spec = m_Framebuffer->GetSepcification();
	...
	{
		//以下代码来自于Imagui_demo.cpp中的ShowExampleAppDockSpace(bool* p_open)函数，并做了相应的简化
		...
		ImGui::Separator();  //用于在界面上绘制一条水平分隔线，通常用于视觉上分隔不同的控件组，提高界面的可读性。
		...
		//&viewportPanelSize获取viewportPanelSize变量的地址
		//(glm::vec2*)将这个地址强制转换为glm::vec2* 类型的指针
		//* 对转换后的指针进行解引用，得到一个glm::vec2类型的值
