...
public:
	static void Init()
	{
		s_RanderEngine.seed(std::random_device()());
	}
	static float Float()
	{
		return (float)s_Distribution(s_RanderEngine) / (float)std::numeric_limits<uint32_t>::max();
	}
private:
	static std::mt19937 s_RanderEngine;
	static std::uniform_int_distribution<std::mt19937::result_type> Random::s_Distribution;
};

std::mt19937 Random::s_RanderEngine;
